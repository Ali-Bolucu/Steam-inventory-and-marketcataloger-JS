document.addEventListener(
  "DOMContentLoaded",
  function () {
    var checkPageButton = document.getElementById("uptade");

    checkPageButton.addEventListener("click", function () {
      var retrievedObject = localStorage.getItem("market_h");
      let market_h = JSON.parse(retrievedObject);
      console.log("market_h: ", market_h);
      let fark;
      let fark_c = 500;
      let fark_s = 0;

      fetch(`https://steamcommunity.com/market/myhistory/render/?query=&start=0&count=1`)
        .then(function (aa) {
          return aa.json();
        }, false)
        .then(function (dat) {
          let total_pos = dat["total_count"];
        ...
        .then(function (lololo) {
          ...
          function fect_market() {
            fetch(`https://steamcommunity.com/market/myhistory/render/?query=&start=${fark_s}&count=${fark_c}`)
              .then(function (u) {
                return u.json();
              })
              .then(function (data) {
                ...
                let market_history = htmlDoc.getElementsByClassName("market_listing_row market_recent_listing_row");
                let total_pos = data["total_count"];
                market_h[0]["game_name"] = total_pos;

                for (let i = 0; i < market_history.length; i++) {
                  let status = market_history[i].getElementsByClassName("market_listing_left_cell market_listing_gainorloss")[0]["innerText"].trim();
                  //console.log(status);

                  let image = market_history[i].getElementsByTagName("img")[0].attributes[1].nodeValue;
                  //console.log(image);

                  let price = market_history[i].getElementsByClassName("market_listing_price")[0].innerText.trim();
                  //console.log(price);

                  let card_name = market_history[i].getElementsByClassName("market_listing_item_name")[0].innerText;
                  console.log(card_name);

                  let game_name = market_history[i].getElementsByClassName("market_listing_game_name")[0].innerText;
                  //console.log(game_name);

                  let date_acted = market_history[i]
                    .getElementsByClassName("market_listing_right_cell market_listing_listed_date can_combine")[0]
                    .innerText.trim();
                  //console.log(date_acted);

                  let date_listed = market_history[i]
                    .getElementsByClassName("market_listing_right_cell market_listing_listed_date can_combine")[1]
                    .innerText.trim();
                  ...
                  for (let container = 0; container < market_h.length; container++) {
                    ...
                    if (market_h[container]["game_name"] == game_name) {
                      for (let contain_card = 0; contain_card < market_h[container]["cards"].length; contain_card++) {
                        ...
                        else if (contain_card + 1 == max_con && market_h[container]["cards"][contain_card]["card_name"] != card_name) {
                          //console.log("_________________");
                    ...
                    else if (market_h.length == container + 1 && market_h[container]["game_name"] != game_name) {
                ...
                document.getElementById("deneme").innerHTML += "WAIT! : " + fark_s;
                ...
                if (fark > fark_s) {
                  ...
                  document.getElementById("deneme").innerHTML = "";
                  document.getElementById("deneme").innerHTML += "Uptade is Done! : " + fark;
